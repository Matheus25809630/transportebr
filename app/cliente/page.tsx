...
export default function ClientePortal() {
  ...
  // 1. Autenticação e Monitoramento de Cargas em Tempo Real
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        setUser(currentUser);
        // Ouve as cargas deste cliente para atualizar o painel automaticamente 
        const q = query(
  ...
  // 2. Integração com a API de IA (Gemini) [cite: 2322, 2382]
  const processarMensagem = useCallback(async (texto: string, imagemBase64?: string) => {
    ...
    try {
      ...
      // Se a IA extrair dados de NF-e, salvamos com a taxa oculta de 0.5% [cite: 2357]
  ...
  // 3. Scanner de NF-e [cite: 2812, 2816]
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* COLUNA ESQUERDA (8 col): Gestão e Mapa  */}
        <div className="lg:col-span-8 space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            <button 
              onClick={() => setIsScanning(true)}
              className="bg-white p-6 rounded-[32px] border-2 border-dashed border-slate-200 hover:border-[#EA1D2C] transition-all flex flex-col items-center gap-3 group"
            >
          ...
          {/* MAPA DE RASTREIO ESTILO UBER [cite: 2544] */}
        ...
        {/* COLUNA DIREITA (4 col): Chatbot e Histórico [cite: 2227, 2545] */}
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-slate-900 rounded-[40px] flex flex-col h-[450px] shadow-2xl overflow-hidden border border-white/10">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              ...
              {isScanning && (
                <div id="reader" className="w-full overflow-hidden rounded-2xl border-2 border-[#EA1D2C] bg-white"></div>
              )}
              <div ref={chatEndRef} />
            </div>
          ...
          {/* ÚLTIMAS CARGAS (Lojista) [cite: 2545] */}
